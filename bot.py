...
@dp.callback_query(lambda c: c.data == "repeat_seedance_boudoir")
async def repeat_seedance_boudoir(callback: CallbackQuery):
    ...
    user_references[callback.from_user.id] = {
        ...
        "video_audio": False,
        "repeat_seedance_prompt": (
            "Используй человека с загруженного фото как главного персонажа. "
            "Сохрани его узнаваемость, лицо, внешность и основные индивидуальные черты. "

            "Формат и стиль: вертикальный будуар-рилс бэкстейдж 9:16, "
            "студия на белом циклорамном фоне, мягкий серый градиент, аккуратная тень под телом. "
            "Склейки между позами каждые примерно 1 секунду. "
            "Мягкий бьюти-свет с бликами на коже. Длительность ровно 10 секунд. "

            "Образ: роскошный чёрный кружевной комплект, бра-балконетт с золотой цепочкой, "
            "кружевные трусики с harness-системой, кожаные ремешки с золотыми пряжками "
            "на талии, бёдрах и ногах, золотые цепочки-подвески. "

            "0:00–0:01 — полулежит на боку на полу спиной к камере, опора на предплечье, "
            "ноги вытянуты вместе, волосы по спине. "
            "0:01–0:02 — на коленях сбоку, глубокий прогиб назад, голова запрокинута. "
            "0:02–0:03 — крупный план снизу, голова наклонена, ладони скользят по животу и талии. "
            "0:03–0:04 — макро бедра, пальцы слегка оттягивают чёрный ремешок-подвязку. "
            "0:04–0:05 — стоит в 3/4, лёгкий прогиб спины, прямой взгляд в камеру. "
            "0:05–0:06 — на полу на другом боку, полулежит спиной к камере, ноги вытянуты. "
            "0:06–0:07 — на коленях сбоку, поднимает руки и собирает волосы на затылке. "
            "0:07–0:08 — крупный план корпуса сбоку, ладонь на бедре, видны ремешки и золотые цепи. "
            "0:08–0:09 — ложится на живот или бок спиной к камере, голова на предплечье. "
            "0:09–0:10 — держит финальную позу лёжа. "

            "Реалистичная кожа, детальное кружево и фурнитура, естественная динамика волос, "
            "мягкая студийная тень. Камера статична. Без текста и надписей. "
            "Чистый белый цикл без реквизита."
        )
    }

    await callback.message.answer(
        "📷 Отправь своё фото.\n\n"
        "Я использую его для создания этого видео с тобой."
    )
...
async def receive_seedance_photo(message: Message):
    ...
    if len(data["video_images"]) >= 9:
        await message.answer("⚠️ Можно добавить максимум 9 фото.")
